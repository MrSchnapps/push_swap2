...
/*   Updated: 2021/03/10 19:57:53 by judecuyp         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	b_to_a(t_swap *a, t_swap *b)
{
	...
	{
		max = (int)(b->part->t_i[b->part->len - 1] -
			b->part->t_i[b->part->len - 2]);
		while (++i < max)
			pa(a, b);
		if (max > 3)
...
int		split_rot(t_swap *a, t_swap *b, int rot)
{
	if (b->len > 0)
...
int		split_a(t_swap *a, t_swap *b)
{
	...
	rot = 0;
	min = a->part->len;
	if (get_median(a->stack, min, a->len, &median))
		return (MERR);
	i = (int)(a->len - min);
	while (i > 0)
	{
